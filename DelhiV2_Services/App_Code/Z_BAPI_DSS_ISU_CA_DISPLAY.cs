...
/// <summary>
/// Summary description for Z_BAPI_DSS_ISU_CA_DISPLAY
/// </summary>
public class Z_BAPI_DSS_ISU_CA_DISPLAY
{
    ...
    public DataTable makeCADetailsDataTable()
    {
        ...
        dcol = new DataColumn();
        dcol.DataType = System.Type.GetType("System.String");
        dcol.ColumnName = "TARIFTYP";
        dt.Columns.Add(dcol);

        dcol = new DataColumn();
        dcol.DataType = System.Type.GetType("System.String");
        dcol.ColumnName = "WERT1";
        dt.Columns.Add(dcol);
    ...
    //Function to make a table with the same structure as BAPIRET2TABLE
    ...
    public DataTable makeCADEtails(DataTable rawTable)
    {
        DataTable ErrorDataTable = makeCADetailsDataTable();
        if (rawTable.Rows.Count > 0)
        {
            for (int intRowCnt = 0; intRowCnt < rawTable.Rows.Count; intRowCnt++)
            {
                CADEtailsAddRow(ErrorDataTable, rawTable.Rows[intRowCnt][0].ToString(), rawTable.Rows[intRowCnt][1].ToString(), rawTable.Rows[intRowCnt][2].ToString(),
                                    ...
                                    rawTable.Rows[intRowCnt][23].ToString(), GetDateFormate_YYYYMMDD1(rawTable.Rows[intRowCnt][24].ToString()), GetDateFormate_YYYYMMDD1(rawTable.Rows[intRowCnt][25].ToString()), rawTable.Rows[intRowCnt][26].ToString(),
                                    ...
                                    rawTable.Rows[intRowCnt][31].ToString(), rawTable.Rows[intRowCnt][32].ToString(), rawTable.Rows[intRowCnt][33].ToString(), GetDateFormate_YYYYMMDD(rawTable.Rows[intRowCnt][34].ToString()),
                                    ...
                                    rawTable.Rows[intRowCnt][39].ToString(), rawTable.Rows[intRowCnt][40].ToString(), rawTable.Rows[intRowCnt][41].ToString());
            }
    ...
    public void CADEtailsAddRow(DataTable dt, string DATA1, string DATA2, string DATA3, string DATA4, string DATA5, string DATA6, string DATA7, string DATA8, string DATA9, string DATA10,
        ...
        string DATA33, string DATA34, string DATA35, string DATA36, string DATA37, string DATA38, string DATA39, string DATA40, string DATA41, string DATA42)
    {
        DataRow dr = dt.NewRow();
        dr["Ca_Number"] = DATA1;
        dr["Bp_Number"] = DATA2;
    ...
    //Function to populate a previously formed Structured DataTable with data present in another table called rawDataTable
